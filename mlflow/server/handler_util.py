    ...
    if _jenkins_clients is None:
        ...
        for env_name in envs_list:
            ...
            if reduce(lambda a, b: a and b, map(lambda a: a is not None, _config)):
                ...
                _logger.log(
                    logging.INFO,
                    "Jenkins client for %s : %s",
                    env_name,
                    str(_jenkins_clients[env_name]),
                )
...
def _validate_string(value: str, human_readable_name: str, error_suffix: str = ""):
    if value is None or value == "":
        raise MlflowException(
            human_readable_name + " cannot be empty for deployment." + error_suffix,
...
def deploy_model_version(
    ...
    _validate_string(
        oyo_team_name,
        "Team Name",
        " Add team name in the registered model page so it can be reused in all versions",
    )
    ...
    if client is None:
        raise MlflowException(
            "Jenkins client not found. Contact administrator for further details.", PUBLIC
        )
    ...
    if response.status_code != 201:
        raise MlflowException(
            "Jenkins job Trigger failed with status code: {}".format(response.status_code), PUBLIC
        )
